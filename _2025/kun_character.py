...
class test_character(InteractiveScene):
    def construct(self):
        ...
        dot1=Dot((-3,2,0))
        dot2=Dot((3,2,0))
        kun1=Kun().to_corner(DL)
        kun2=YellowChicken().to_corner(DR)
        text1=Text("学线性代数,到底在学什么？？",font="SJsuqian")
        text2=Text("其实就是学怎么解线性方程组",font="SJsuqian")
        text3=Text("为什么要学习解线性方程组",font="SJsuqian")
        text4=Text("因为它无处不在",font="SJsuqian")
        bubble1=Bubble(text1).pin_to(kun1.right_eye)
        bubble2=Bubble(text2).flip().pin_to(kun2.left_eye)
        bubble3=Bubble(text3).pin_to(kun1.right_eye)
        bubble4=Bubble(text4).flip().pin_to(kun2.left_eye)
        text3.set_stroke(WHITE,1,1)
        text4.set_stroke(WHITE,1,1)
        self.add(kun1,kun2)
        self.play(kun1.animate.close_eyes().set_anim_args(rate_func=there_and_back))
        self.play(Write(bubble1),kun1.animate.look_at(kun2).peace())
        self.wait()
        self.play(Write(bubble2),FadeOut(bubble1),kun2.animate.look_at(kun1))
        self.wait()
        self.play(Write(bubble3),FadeOut(bubble2),kun1.animate.shock())
        self.wait()
        self.play(Write(bubble4),FadeOut(bubble3),kun2.animate.smile().raise_hand())
        self.wait()
